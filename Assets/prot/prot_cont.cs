...
public class prot_cont : MonoBehaviour
{
    ...
    public void remove(){
        Vector3 pos;
        prot_npc.transform.position = GameManager.Get_Stage_Maneger().GetComponent<StageManager>().VectorReturn(NPC_pos.x,NPC_pos.y);
        pos = prot_npc.transform.position;
        pos.y = 1;
        prot_npc.transform.position= pos;
        prot_tree.transform.position = GameManager.Get_Stage_Maneger().GetComponent<StageManager>().VectorReturn(tree_pos.x,tree_pos.y);
